...
def save_to_database(data, prediction_result):
    ...
    try:
        ...
        registro = {
            ...
            "parametros_monitoramento": {k: data.get(k, 0) for k in data.keys() if k.startswith(('baseline', 'accelerations', 'fetal', 'uterine', 'light', 'severe', 'prolongued', 'abnormal', 'mean', 'percentage', 'histogram'))},
            "resultado_ml": prediction_result,
            "saude_feto": {"status_saude": status_saude, "confidence_value": prediction_result['confidence'], "nivel_risco": nivel_risco},
...
def predict():
    ...
    try:
        ...
        # Salvar no banco
        record_id = save_to_database(data, response)
        if record_id:
